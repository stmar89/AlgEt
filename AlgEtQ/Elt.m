...
{Given sequences a and b, such that #a eq #b, returns &+[a[i]*b[i] : i in [1..#a]]. .}
    ...
    as:=[ Components(ai) : ai in a ];
    bs:=[ Components(bi) : bi in b ];
    N := #a;
    N_comp:=#bs[1];
...
///ditto
intrinsic _DotProduct(a::SeqEnum[RngQZElt], b::SeqEnum[AlgEtQElt]) -> AlgEtQElt
{ " } // "
    A := Universe(b);
    require #a eq #b : "The sequences don't have the same size";
    bs := [ Components(bi) : bi in b ];
    N := #a;
    N_comp := #bs[1];
    return CreateAlgEtQElt(A,< &+[ a[i]*bs[i][c] : i in [1..N] ] : c in [1..N_comp]>);
end intrinsic;

intrinsic _DotProduct(a::SeqEnum[AlgEtQElt], b::SeqEnum[RngQZElt]) -> AlgEtQElt
{ " } // "
    return _DotProduct(b, a);
end intrinsic;

intrinsic DotProduct(a::SeqEnum, b::SeqEnum) -> Any
{ " } // "
    types := {ElementType(a), ElementType(b)};
    if AlgEtQElt in types and types subset {RngIntElt, FldRatElt, AlgEtQElt} then
        // Use the optimized intrinsic
        return _DotProduct(a, b);
    else
        // Fallback to the generic intrinsic
        require #a eq #b : "The sequences don't have the same size";
...
///
/// // We conclude this example by showing the use of DotProduct and its timings advantages:
/// N:=10^5;
/// elts1:=[ a+i : i in [1..N] ];
/// elts2:=[ a-i : i in [1..N] ];
