...
/// We observed that applying Hash to this sequence can easily produce collisions.
...
///## Equality
/// Equality of orders is perfomed using the `Hash` attribute described above.
...
/// Returns a random element of a given order  with coefficients with respect to the stored $\mathbb{Z}$-basis bounded by `bd`. One can allow zero-divisors using the parameter `ZeroDivisorsAllowed`, which is set to false by default.
...
/// Returns a random element of a given order. The coefficients with respect to the stored $\mathbb{Z}$-basis can be bounded by setting the parameter `CoeffRange`. One can allow zero-divisors using the parameter `ZeroDivisorsAllowed`, which is set to false by default.
...
/// Given an order $T$ computes its index in the free $\mathbb{Z}$-module generated by the absolute basis of the algebra.
intrinsic Index(T::AlgEtQOrd) -> FldRatElt
{Given an order T computes its index in the free Z-module generated by the absolute basis of the algebra as .}
