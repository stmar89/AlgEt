...
{Returns the order generated as an algebra over ZZ by a given sequence of elements. The parameter Check (default 100) determines how many times the program tries to obtain a multiplicatively closed lattice by adding the product of the given generators. If Check 0 then this step is skipped. The vararg CheckIsKnownOrder determines whether the program checks if the order is already known, i.e. in the attribute KnownOrders of the algebra. The default value is true. The stored ZBasis is LLL-reduced.}
    A:=Universe(gens);
    ...
    if Check gt 0 then
        ...
        repeat
            ...
            gens0:=MatrixQtoA(A,P);
            fail +:=1;
            if P cmpeq P_old then
                if Rank(P_old) eq dim then
                    go:=true;
                else
                    error "The algebra over ZZ generated by the given set is not a full ZZ-lattice in the étale algebra.";
                end if;
            end if;
        until go or fail gt Check;
        if fail gt Check then
            error "The program failed to compute the Z-algebra generated by the given set. Try to increase the parameter Check.";
        end if;
    ...
    ZBasisLLL(R); // for better coefficients
    return R;
end intrinsic;
...
{Returns the monogenic order of the étale algebra, which depends on the stored primitive element.}
    if not assigned A`EquationOrder then
        ...
        E`Generators:=[pow[2]]; // I want the generator set to be small, so I put only the primitive element
        A`EquationOrder:=E;
    end if;
