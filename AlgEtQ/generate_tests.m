...
// Based on https://github.com/stmar89/AlgEt/blob/0e293da57edfd90f57380ac9d684b7a1d4a7090a/dev/fast_tests_AlgEtQ_make/magma_make_fast_tests_AlgEtQ.txt
/*
...
The tests can then be run in parallel. For example:
    MAGMA_USER_SPEC="../../../package/Ring/AlgEtQ/AlgEtQ.spec"; ls test_* | parallel magma
*/
...
generate_test_content_from_file:=function(folder, filename)
    ...
    // We remove the unnecessary indentation
    lines := Split(inp_local,"\n" : IncludeEmpty);
    indentation := Min([count_indentation(line) : line in lines | line ne ""]);
    lines := [line eq "" select line else line[indentation+1..#line] : line in lines | line ne ""];
    inp_local := Join(lines,"\n");
    out := Sprintf("// Tests from %o/%o\n", folder, filename);
    out cat:= "time_start_local := Cputime();\n";
    out cat:= inp_local;
    out cat:= "\nSprintf(\" time: %o\", Cputime(time_start_local));\n";
    return out;
end function;
