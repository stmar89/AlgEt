...
- v1.0.1 : June 5, 2026
  ...
  - Breaking changes and additional functionalities:
    ...
    - The precision parameter for various intrinsic is now consistently called `Precision`.
    - Privatized internal intrinsics to compute intermediate ideal with underscore prefix (`_MinimalIntermediateIdeals`, etc.).
    - New public `IntermediateIdeals` intrinisc with boolean parameters `Maximal`, `Minimal`, `PrescribedMultiplicatorRing` returning only maximal, minimal and fildered by multiplicator ring intermediate ideals.
