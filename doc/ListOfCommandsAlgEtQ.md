...
Inclusion of element in the order.
...
Random element of O. The Coefficients are bounded by the positive integer bd. One can allow zero-divisors using the optional argument "ZeroDivisorsAllowed", which is set to false by default.
...
This procedure checks wheter the order R is already in the list of known orders of the algebra A containing R. If so then it replaces R with the copy stored in the attribute KnownOrders. If not it adds it to KnownOrders. This is done to avoid creating multiple copies of the same order.
